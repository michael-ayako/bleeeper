...
$sql="SELECT * FROM group_req_tb WHERE group_id='$gid' AND state='1'";
...
echo '<div class="search-field"><img src="'.$dir.'"/><div class="details">'.$fname.' '.$mname.' '.$lname.'<br/><br/>'.$uname.'<br/><br/>'.$email.'<br/><br/></div><div class="area">Sent on:<br/><br/>'.$res2["date_and_time"].'</div>
<input type="button" id="invite_grp_user" value="Allow" onClick="funcallow_req('.$id.','.$gid.');return false;"/>
<input type="button" id="block" value="Deny" onClick="funcdeny_req('.$id.','.$gid.');return false;"/>
</div>';
...
if($x==0)
echo "<div class='failed' >There are no new requests</div>";
